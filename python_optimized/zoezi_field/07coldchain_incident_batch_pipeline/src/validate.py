...
# exposure_minutes is less than or equal to 0.
...
def validate_data(event):
    ...
    if product_category is None:
        ...
        validated.append(
            f"product_category must be one of {'; '.join(ALLOWED_PRODUCT_CATEGORIES)}"
        )
    ...
    if delivery_status is None:
        ...
        validated.append(
            f"delivery_status should be one of {'; '.join(ALLOWED_DELIVERY_STATUSES)}"
        )
